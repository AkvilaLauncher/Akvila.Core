...
using Akvila.Core.Helpers.Mirrors;
using Akvila.Core.Services.System;
using Akvila.Models.CmlLib;
...
namespace Akvila.Core.Helpers.Game;
...
public class GameDownloader {
    ...
    public async Task<string> DownloadGame(GameLoader loader, string version, string? launchVersion,
        ...
        if (!_downloadMethods.ContainsKey(loader)) {
            await _notifications.SendMessage("Ошибка", "Attempting to create a profile with an invalid loader", NotificationType.Error);
    ...
    private async Task<string> DownloadVanilla(string version, string? launchVersion,
        ...
        foreach (var launcher in _launchers.Values)
            try {
                ...
                var title =
                    $"{_profile.Name} {launcher.RulesContext.OS.Name} [{launcher.RulesContext.OS.Arch}] skipped";
                var details =
                    $"Creating a {_profile.Name} profile is skipped for the operating system {launcher.RulesContext.OS.Name} " +
                    $"with the  {launcher.RulesContext.OS.Arch} architecture. " +
                    $"The Minecraft version {_profile.GameVersion} does not support this configuration.";
                ...
                var message =
                    $"Client for {launcher.RulesContext.OS.Name}, {launcher.RulesContext.OS.Arch} has not been installed!";
    ...
    private async Task<string> DownloadForge(string version, string? launchVersion,
        CancellationToken cancellationToken) {
        ...
        foreach (var launcher in _launchers.Values)
            try {
                ...
                if (bestVersion is null) {
                    await _notifications.SendMessage("Error", "Failed to determine the version to download", NotificationType.Error);
    ...
    private async Task CheckBuildJava() {
        ...
        if (!Directory.Exists(javaDirectory) || !File.Exists(javaPath)) {
            ...
            // ToDo: Take the ping to another class
    ...
    public async Task<Process> GetProcessAsync(IStartupOptions startupOptions, IUser user, bool needDownload,
        ...
        if (!_launchers.TryGetValue($"{startupOptions.OsName}/{startupOptions.OsArch}", out var launcher)) {
            await _notifications.SendMessage("Error", "The selected operating system is not supported", NotificationType.Error);
        ...
        if (needDownload)
            foreach (var anyLauncher in _launchers.Values)
                try {
                    ...
                    var message =
                        $"Failed to restore profile {_profile.Name}, {anyLauncher.RulesContext.OS.Name}, {anyLauncher.RulesContext.OS.Arch}";
                    await _notifications.SendMessage(message, exception);
                    _exception.OnNext(exception);
                    _loadLog.OnNext(message);
                }
