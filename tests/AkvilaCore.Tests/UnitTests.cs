...
public class Tests {
    ...
    private AkvilaManager AkvilaManager { get; } =
        new(new AkvilaSettings("AkvilaLauncher", "gfweagertghuysergfbsuyerbgiuyserg", httpClient: new HttpClient()) {
            TextureServiceEndpoint = "http://akvila-web-skins:8085"
        });

    [OneTimeSetUp]
    public async Task Setup() {
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(CheckProfileName)
                           ?? await AkvilaManager.Profiles.AddProfile(CheckProfileName, CheckProfileName,
    ...
    public void InitializeTest() {
        Assert.Multiple(() => {
            Assert.That(AkvilaManager.LauncherInfo.InstallationDirectory, Is.Not.Empty);
            Assert.That(AkvilaManager.LauncherInfo.BaseDirectory, Is.Not.Empty);
            Assert.That(AkvilaManager.LauncherInfo.Name, Is.Not.Empty);
        });
    }
    ...
    public async Task Create_LiteLoader_Profile() {
        ...
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(name)
                           ?? await AkvilaManager.Profiles.AddProfile(name, CheckProfileName, "1.7.10", "1.7.10_04",
    ...
    public async Task Create_Vanilla_Profile() {
        ...
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(name)
                           ?? await AkvilaManager.Profiles.AddProfile(name, CheckProfileName, "1.20.1", string.Empty,
                               GameLoader.Vanilla,
                               string.Empty,
                               string.Empty)
                           ?? throw new Exception("Failed to create profile instance");
        ...
        Assert.Multiple(async () => {
            Assert.That(
                await AkvilaManager.Profiles.CanAddProfile(name, "1.20.1", string.Empty, GameLoader.Vanilla),
                Is.False);
        });
    }
    ...
    public async Task Create_Forge_Profile() {
        ...
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(name)
                           ?? await AkvilaManager.Profiles.AddProfile(name, CheckProfileName, "1.7.10", "10.13.4.1614",
                               GameLoader.Forge,
                               string.Empty,
                               string.Empty)
                           ?? throw new Exception("Failed to create profile instance");
        ...
        Assert.Multiple(async () => {
            Assert.That(
                await AkvilaManager.Profiles.CanAddProfile(name, "1.7.10", string.Empty, GameLoader.Forge),
                Is.False);
        });
    }
    ...
    public async Task Create_NeoForge_Profile() {
        ...
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(name)
                           ?? await AkvilaManager.Profiles.AddProfile(name, name, "1.20.4", "neoforge-20.4.237",
                               GameLoader.NeoForge,
                               string.Empty,
                               string.Empty)
                           ?? throw new Exception("Failed to create profile instance");
        ...
        Assert.Multiple(async () => {
            Assert.That(
                await AkvilaManager.Profiles.CanAddProfile(name, "1.7.10", string.Empty, GameLoader.NeoForge),
                Is.False);
        });
    }
    ...
    public async Task Create_Fabric_Profile() {
        ...
        _testGameProfile = await AkvilaManager.Profiles.GetProfile(name)
                           ?? await AkvilaManager.Profiles.AddProfile(name, CheckProfileName, "1.20.1", "0.16.0",
                               GameLoader.Fabric,
                               string.Empty,
                               string.Empty)
                           ?? throw new Exception("Failed to create profile instance");
        ...
        Assert.Multiple(async () => {
            Assert.That(
                await AkvilaManager.Profiles.CanAddProfile(name, "1.7.10", string.Empty, GameLoader.Fabric),
                Is.False);
        });
    }
    ...
    public async Task CreateServer() {
        var server = await AkvilaManager.Servers.AddMinecraftServer(_testGameProfile, ServerName, "127.0.0.1", 25565);
    ...
    public async Task RemoveServer() {
        await AkvilaManager.Servers.RemoveServer(_testGameProfile, ServerName);
    }

    [Test]
    [Order(6)]
    public async Task Get_All_Minecraft_Versions() {
        var versions = await AkvilaManager.Profiles.GetAllowVersions(GameLoader.Vanilla, string.Empty);
    ...
    public async Task Get_Forge_1_7_10_Versions() {
        var versions = await AkvilaManager.Profiles.GetAllowVersions(GameLoader.Forge, "1.7.10");
    ...
    public async Task Get_Fabric_1_20_1_Versions() {
        var versions = await AkvilaManager.Profiles.GetAllowVersions(GameLoader.Fabric, "1.20.1");
    ...
    public async Task Get_LiteLoader_1_7_10_Versions() {
        var versions = await AkvilaManager.Profiles.GetAllowVersions(GameLoader.LiteLoader, "1.7.10");
    ...
    public async Task Remove_Profile() {
        ...
        var checkProfile = await AkvilaManager.Profiles.GetProfile(name);
    ...
    public async Task DownloadProfile() {
        _testGameProfile = await AkvilaManager.Profiles.GetProfile("HiTech")
                           ?? await AkvilaManager.Profiles.AddProfile("HiTech", "HiTech", "1.20.1", string.Empty,
                               GameLoader.Vanilla,
                               string.Empty,
                               string.Empty)
                           ?? throw new Exception("Failed to create profile instance");
    ...
    public async Task CheckInstallDotnet() {
        var isInstalled = await AkvilaManager.System.InstallDotnet();
    ...
    public async Task Build_launcher() {
        var launcherVersions = await AkvilaManager.Launcher.GetVersions();

        var version = launcherVersions.First();
        bool isBuild = false;

        var logsDisposable = AkvilaManager.System.DownloadLogs.Subscribe(logs => {
            Console.WriteLine(logs);
            Debug.WriteLine(logs);
        });

        var eventDisposable = AkvilaManager.Launcher.BuildLogs.Subscribe(logs => {
            Console.WriteLine(logs);
            Debug.WriteLine(logs);
        });

        if (await AkvilaManager.System.InstallDotnet()) {
            if (!AkvilaManager.Launcher.CanCompile(version, out var _)) {
                await AkvilaManager.Launcher.Download(version, "http://localhost:5000", "AkvilaLauncher");
            }

            if (AkvilaManager.Launcher.CanCompile(version, out var message)) {
                Console.WriteLine(message);
                Debug.WriteLine(message);
                isBuild = await AkvilaManager.Launcher.Build(version, ["win-x64"]);
            }
        }
        ...
        Assert.Multiple(() => {
            Assert.That(AkvilaManager.Launcher.CanCompile(version, out _), Is.True);
    ...
    public async Task GetJavaVersions() {
        var versions = await AkvilaManager.System.GetJavaVersions();
    ...
    public async Task GetMods_By_Modrinth() {
        ...
        var profile = await AkvilaManager.Profiles.GetProfile(name)
                      ?? await AkvilaManager.Profiles.AddProfile(name, name, "1.20.1", string.Empty, GameLoader.Forge,
                          string.Empty,
                          string.Empty)
                      ?? throw new Exception("Failed to create profile instance");

        var mods = await AkvilaManager.Mods.FindModsAsync(
            profile.Loader,
            profile.GameVersion,
            ModType.Modrinth,
            string.Empty,
    ...
    public async Task GetModsInfo_By_Modrinth() {
        ...
        var mod = (await AkvilaManager.Mods.FindModsAsync(
            profile.Loader,
            profile.GameVersion,
            ModType.Modrinth,
            string.Empty,
        ...
        var mods = await AkvilaManager.Mods.GetInfo(mod.Id, mod.Type);
    ...
    public async Task AddModToProfile_From_Modrinth() {
        ...
        var profile = await AkvilaManager.Profiles.GetProfile(name)
                      ?? await AkvilaManager.Profiles.AddProfile(name, name, "1.20.1", string.Empty, GameLoader.Fabric,
                          string.Empty,
                          string.Empty)
                      ?? throw new Exception("Failed to create profile instance");
        ...
        var modInfo = await AkvilaManager.Mods.GetInfo(mod.Id, mod.Type);
        ...
        await profile.AddMod(Path.GetFileName(version),
            await AkvilaManager.LauncherInfo.Settings.HttpClient.GetStreamAsync(version));
    ...
    public async Task GetNewsForVk() {
        var vkProvider = new VkNewsProvider("", AkvilaManager);

        await AkvilaManager.Integrations.NewsProvider.AddListener(vkProvider);

        await AkvilaManager.Integrations.NewsProvider.RefreshAsync();
    ...
    public async Task GetNewsForUnicore() {
        ...
        await AkvilaManager.Integrations.NewsProvider.AddListener(unicoreProvider);
    ...
    public async Task GetNewsForAzuriom() {
        ...
        await AkvilaManager.Integrations.NewsProvider.AddListener(azuriomProvider);
    ...
    public async Task GetNewsForCustom() {
        ...
        await AkvilaManager.Integrations.NewsProvider.AddListener(customProvider);
