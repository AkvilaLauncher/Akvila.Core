...
using Akvila.Core.Exceptions;
using Akvila.Core.Helpers.Game;
...
namespace Akvila.Core.Helpers.Profiles {
    public partial class ProfileProcedures : IProfileProcedures {
        ...
        // TODO: Replace with Akvila repository URL when available
        ...
        public async Task<bool> CanAddProfile(string name, string version, string loaderVersion,
            ...
            switch (dtoGameLoader) {
                case GameLoader.Undefined: break;
                case GameLoader.Vanilla: return versions.Any(c => c.Equals(version));
                case GameLoader.Forge: return versions.Any(c => c.Equals(loaderVersion));
                case GameLoader.Fabric: return versions.Any(c => c.Equals(loaderVersion));
                case GameLoader.LiteLoader: return versions.Any(c => c.Equals(loaderVersion));
                case GameLoader.NeoForge: return versions.Any(c => c.Equals(loaderVersion));
                case GameLoader.Quilt: return versions.Any(c => c.Equals(loaderVersion));
                default: throw new ArgumentOutOfRangeException(nameof(dtoGameLoader), dtoGameLoader, null);
            }
        ...
        public async Task<bool> ValidateProfileAsync(IGameProfile baseProfile) {
            // ToDo: Make profile validation via schema
            // 27.05.2025 playgamesgo: what schema?
        ...
        public async Task<IGameProfileInfo?> RestoreProfileInfo(
            ...
            try {
                ...
                _bugTracker.CaptureException(exception);
                throw new Exception($"Failed to restore game profile. {exception}");
            } finally {
                        ...
                        switch (_launcherInfo.StorageSettings.StorageType) {
                            ...
                            case StorageType.S3:
                                ...
                                if (await _akvilaManager.Files.CheckFileExists("profiles", file.Hash) == false) {
                                    await _akvilaManager.Files.LoadFile(File.OpenRead(filePath), "profiles", file.Hash,
                                        tags);
                                }
        ...
        public async Task<IEnumerable<string>> GetAllowVersions(GameLoader gameLoader, string? minecraftVersion) {
            try {
                ...
                Console.WriteLine(e);
                throw new VersionNotLoadedException("Failed to get a list of versions for this loader. " + e.Message);
            }
        }
        ...
        private async Task UpdateProfilesService(GameProfile gameProfile) {
            foreach (var server in gameProfile.Servers) {
                server.ServerProcedures = _akvilaManager.Servers;
            ...
            gameProfile.GameLoader = new GameDownloaderProcedures(_launcherInfo, _storageService, gameProfile,
                _notifications, _akvilaManager.BugTracker);
