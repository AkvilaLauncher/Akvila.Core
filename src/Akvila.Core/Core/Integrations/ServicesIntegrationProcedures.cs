...
public class ServicesIntegrationProcedures(
    ...
    public INewsListenerProvider NewsProvider { get; set; } =
        new NewsListenerProvider(TimeSpan.FromMinutes(1), storage, bugTracker, akvilaManager);
    ...
    public Task<IEnumerable<IAuthServiceInfo>> GetAuthServices() {
        return Task.FromResult(new List<IAuthServiceInfo> {
            ...
            new AuthServiceInfo("Microsoft", AuthType.Microsoft)
        }.AsEnumerable());
    }
